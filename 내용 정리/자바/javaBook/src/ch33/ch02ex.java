...
class Calc {
	...
	Calc() {
		...
		sum = (items) -> Arrays.stream(items).reduce(0, (sum, el) -> sum + el);
		// Functional sub 에 대한 스트림 & 람다식 완성 할 것
		sub = (items) -> Arrays.stream(items).boxed().sorted((x, y) -> y - x).reduce((sub, el) -> sub - el).orElse(0);
		// Functional mul 에 대한 스트림 & 람다식 완성 할 것
		mul = (items) -> Arrays.stream(items).reduce((mul, el) -> mul * el).orElse(0);
		// Functional div 에 대한 스트림 & 람다식 완성 할 것
		div = (items) -> Arrays.stream(items).boxed().sorted((x, y) -> y - x).reduce((div, el) -> div / el).orElse(0);
		// 조건
...
public class ch02ex {
	...
	public static void main(String[] args) {
		...
		System.out.println("나눗셈 :" + calc.div.execute(2, 3, 2, 100));
